...
try:
	infile = sys.argv[1]
	if not infile.endswith('.fasta'):
		raise ValueError("Not a FASTA file")
except ValueError:
	print("File needs to be a FASTA file ending in .fasta")
except IndexError:
	...
	try:
		with open(infile,'r') as file, open('Python_08.codons-6frames.nt','w') as writecodons, open('Python_08.translated.aa','w') as writeaas:
			...
			for gene in re.finditer('>(\S*)[ \r\t]*(.*)\n([\w\n]+)',fasta):
				...
				if re.search('[^ATCGN]',sequence):
					raise ValueError("non-ATCGN in sequence")
		...
		print("non-ATCGN found in sequence")
